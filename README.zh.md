...
如果你的公众号调不了草稿箱接口——未认证的个人订阅号经常如此,见[公众号类型](#公众号类型)——排版层照样能用,而且这部分本身就值。
...
能调哪些接口取决于账号类型和认证状态。**未认证的个人订阅号**通常既没有草稿箱也没有素材接口,调用会返回 **errcode 48001「api unauthorized」**。

真遇到时插件会用人话说明。如果你的号就是这种,那就用 `mp_render`,把 HTML 自己粘进后台——这条路完全不需要任何接口权限。
